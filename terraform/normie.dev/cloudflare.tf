resource "cloudflare_zone" "normie_dev" {
  account_id = data.sops_file.cloudflare-secrets.data["account_id"]
  zone       = "normie.dev"
  plan       = "free"
  type       = "full"
}
...
locals {
  ipv4_addresses = toset([for s in data.ovh_vps.keanu_ovh.ips : s if can(cidrnetmask("${s}/0"))])
  ipv6_addresses = toset([for s in data.ovh_vps.keanu_ovh.ips : s if !can(cidrnetmask("${s}/0"))])
}
